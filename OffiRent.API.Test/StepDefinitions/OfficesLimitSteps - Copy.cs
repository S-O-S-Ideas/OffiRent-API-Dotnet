...
namespace OffiRent.API.Test.StepDefinitions
{
    ...
    public class OfficesLimitSteps
    {
        ...
        {
            objectContainer.RegisterTypeAs<OfficeRepository, IOfficeRepository>();
        }

        public OfficesLimitSteps(OfficeService officeService, IObjectContainer objectContainer, OfficeRepository officeRepository)
        {
        ...
        {
            response = _officeService.SaveAsync(newOffice).Result;
        }
        ...
        public async void GivenTheUserHasMoreThanActiveOfficePosts(int p0)
        {
            for (int i = 0; i < p0; i++)
            {
                await _officeService.SaveAsync(newOffice);
